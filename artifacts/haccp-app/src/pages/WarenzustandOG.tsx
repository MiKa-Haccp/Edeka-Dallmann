    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#1a3a6b] text-white text-sm font-bold disabled:opacity-50 hover:bg-[#1a3a6b] transition-colors"
        >
    ...
    <AppLayout>
      <div className="max-w-sm mx-auto px-4 py-10 flex flex-col items-center gap-6">
        ...
        <div className="text-center space-y-1">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Warenzustand Obst & Gemüse</p>
        ...
        {!done ? (
          <div className="w-full bg-white rounded-2xl border border-border shadow-sm p-5 space-y-4">
            ...
            <button
              onClick={handleVerify}
              disabled={loading || pin.length < 4}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[#1a3a6b] text-white text-sm font-bold disabled:opacity-50 hover:bg-[#1a3a6b] transition-colors"
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
              Abzeichnen
            </button>
              ...
              <table className="w-full text-sm border-collapse">
                <thead className="sticky top-0 z-20">
                  <tr className="bg-gradient-to-br from-[#1a3a6b] to-[#2d5aa0] text-white">
                    <th className="px-3 py-3 text-left font-semibold text-xs w-14 sticky left-0 top-0 bg-[#1a3a6b] z-30">Tag</th>
                    {SLOTS.map(s => (
                      ...
                      >
                        <td className="px-3 py-2 sticky left-0 bg-inherit z-10">
                          <div className="flex flex-col items-start leading-none">
                            ...
                            <span className={`text-[10px] font-medium mt-0.5 ${isClosed ? "text-slate-400" : isSaturday ? "text-blue-400" : "text-muted-foreground"}`}>{wd}</span>
                          </div>
                        </td>
                        {isClosed ? (
