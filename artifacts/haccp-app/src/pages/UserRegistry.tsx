...
function RegistrationForm() {
  ...
  const { refetch } = useListUsers({ tenantId: 1 });
  ...
  return (
    <div className="bg-white rounded-2xl border border-border/60 shadow-sm overflow-hidden">
      ...
      {!isAdmin && registeredUsers.length > 0 && (
        <div className="px-6 py-3 bg-amber-50 border-t border-amber-200 flex items-center gap-2 text-sm text-amber-700">
          <Lock className="h-4 w-4 flex-shrink-0" />
          Namen und Geburtsdaten sind nur für angemeldete Administratoren sichtbar.
        </div>
      )}
    </div>
  );
...
function AdminInviteSection() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/admin/invite`, {
        ...
        body: JSON.stringify({ email, tenantId: 1, adminEmail: adminSession?.email }),
      });
      ...
      setInviteResult({ token: data.token, inviteUrl: data.inviteUrl });
      setEmail("");
    } catch {
  ...
  const copyLink = () => {
    ...
    const fullUrl = `${window.location.origin}${import.meta.env.BASE_URL.replace(/\/$/, "")}${inviteResult.inviteUrl}`;
  ...
  return (
    <div className="bg-white rounded-2xl border border-border/60 shadow-sm overflow-hidden">
      <div className="bg-[#1a3a6b] text-white px-6 py-3 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold">Admin einladen</h2>
      </div>

      <div className="p-6">
        <p className="text-sm text-muted-foreground mb-4">
          Laden Sie einen neuen Administrator per E-Mail ein. Der eingeladene Benutzer erhält einen Registrierungslink.
        </p>
        ...
        <form onSubmit={handleInvite} className="flex gap-3 items-end">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
              placeholder="neuer.admin@firma.de"
              required
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="bg-[#1a3a6b] text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-[#1a3a6b]/90 transition-colors disabled:opacity-50 flex items-center gap-2"
          >
        ...
        {inviteResult && (
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl space-y-3">
            ...
            <p className="text-xs text-green-600">
              Teilen Sie diesen Link mit dem eingeladenen Administrator. Der Link ist 7 Tage gültig.
            </p>
            <div className="flex items-center gap-2">
              <code className="flex-1 bg-white border border-green-300 rounded-lg px-3 py-2 text-xs font-mono break-all">
                {window.location.origin}{import.meta.env.BASE_URL.replace(/\/$/, "")}{inviteResult.inviteUrl}
              </code>
              <button
                onClick={copyLink}
