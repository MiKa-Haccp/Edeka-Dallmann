...
import {
  GraduationCap, ShieldAlert, Flame, BookOpen, FileText,
...
type Kategorie = "lebensmittelhygiene" | "arbeitssicherheit" | "brandschutz" | "haccp" | "sonstige";

interface Schulungsnachweis {
  id: number;
  tenantId: number;
  kategorie: Kategorie;
  mitarbeiterName: string;
  bezeichnung: string;
  schulungsDatum: string;
  naechsteSchulung: string;
  anbieter: string;
...
const TABS: {
  ...
  {
    key: "lebensmittelhygiene",
    label: "Lebensmittelhygiene",
    kurzLabel: "LM-Hygiene",
    icon: <BookOpen className="w-4 h-4" />,
    color: "text-blue-600",
    bgColor: "bg-blue-50",
    borderColor: "border-blue-200",
    hinweis: "VO (EG) 852/2004 / LMHV: Alle Mitarbeiter mit Lebensmittelkontakt sind regelmäßig in Hygiene zu schulen. Empfehlung: jährlich.",
  },
  {
    key: "arbeitssicherheit",
    label: "Arbeitssicherheit & Unterweisungen",
    kurzLabel: "Arbeitssich.",
    icon: <ShieldAlert className="w-4 h-4" />,
    color: "text-amber-600",
    bgColor: "bg-amber-50",
    borderColor: "border-amber-200",
    hinweis: "ArbSchG / BGV A1: Sicherheitsunterweisungen sind mindestens jährlich und bei Arbeitsplatzwechsel durchzuführen.",
  },
  {
    key: "brandschutz",
    ...
    hinweis: "ASR A2.2: Brandschutzhelfer-Schulung alle 3–5 Jahre. Evakuierungsunterweisungen jährlich empfohlen.",
  },
  {
    key: "haccp",
    label: "HACCP-Grundlagen",
    kurzLabel: "HACCP",
    icon: <GraduationCap className="w-4 h-4" />,
    color: "text-green-600",
    bgColor: "bg-green-50",
    borderColor: "border-green-200",
    hinweis: "VO (EG) 852/2004 Anhang II Kap. XII: Betreiber stellen sicher, dass Mitarbeiter die HACCP-Grundsätze kennen und anwenden.",
  },
  {
    key: "sonstige",
    label: "Sonstige Schulungen",
    kurzLabel: "Sonstige",
...
function getStatus(naechsteSchulung: string | null) {
  if (!naechsteSchulung) return "ok";
  const d = new Date(naechsteSchulung);
...
function NachweisForm({ kategorie, onSave, onCancel }: {
  kategorie: Kategorie;
  onSave: (fields: Partial<Schulungsnachweis>) => Promise<void>;
  onCancel: () => void;
}) {
  const tab = TABS.find((t) => t.key === kategorie)!;
  const [mitarbeiterName, setMitarbeiterName] = useState("");
  const [bezeichnung, setBezeichnung] = useState("");
  const [schulungsDatum, setSchulungsDatum] = useState("");
  const [naechsteSchulung, setNaechsteSchulung] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await onSave({ kategorie, mitarbeiterName: mitarbeiterName.trim(), bezeichnung: bezeichnung.trim(), schulungsDatum, naechsteSchulung, anbieter: anbieter.trim(), dokumentBase64: dokument, notizen: notizen.trim() });
  ...
  return (
    <div className={`${tab.bgColor} border ${tab.borderColor} rounded-2xl p-5 space-y-4`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Schulungsthema</label>
          <input value={bezeichnung} onChange={(e) => setBezeichnung(e.target.value)} placeholder="z.B. HACCP-Grundlagenkurs, Hygienebelehrung..."
            className="px-3 py-2 rounded-xl border border-border/60 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 transition-all" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Schulungsdatum</label>
          <input type="date" value={schulungsDatum} onChange={(e) => setSchulungsDatum(e.target.value)}
            className="px-3 py-2 rounded-xl border border-border/60 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 transition-all" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Nächste Schulung / Wiederholung</label>
          <input type="date" value={naechsteSchulung} onChange={(e) => setNaechsteSchulung(e.target.value)}
            className="px-3 py-2 rounded-xl border border-border/60 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 transition-all" />
        </div>
      </div>
      <div className="flex flex-col gap-1">
        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Schulungsanbieter / Schulungsleiter</label>
        <input value={anbieter} onChange={(e) => setAnbieter(e.target.value)} placeholder="z.B. EDEKA-Akademie, interner Schulungsleiter..."
          className="px-3 py-2 rounded-xl border border-border/60 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 transition-all" />
      </div>
      <div className="flex flex-col gap-1">
      ...
      <div className="flex flex-col gap-2">
        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Dokument (Teilnahmebestätigung, Foto oder PDF)</label>
        {dokument ? (
          isPdf ? (
...
function NachweisKarte({ n, tab, onDelete, isAdmin }: {
  n: Schulungsnachweis; tab: typeof TABS[0]; onDelete: () => void; isAdmin: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const status = getStatus(n.naechsteSchulung);
  const isPdf = n.dokumentBase64?.startsWith("data:application/pdf");
    ...
    <div className={`bg-white rounded-2xl border-2 overflow-hidden ${status === "abgelaufen" ? "border-red-200" : status === "bald" ? "border-amber-200" : "border-border/40"}`}>
      <button onClick={() => setOpen((p) => !p)}
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-foreground">{n.mitarbeiterName}</p>
          {n.bezeichnung && <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{n.bezeichnung}</p>}
          {n.naechsteSchulung && !n.bezeichnung && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Nächste Schulung {new Date(n.naechsteSchulung).toLocaleDateString("de-DE")}
            </p>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {n.naechsteSchulung && <StatusBadge status={status} />}
          {n.naechsteSchulung && status === "ok" && (
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 text-green-700 flex items-center gap-1 whitespace-nowrap">
              <CheckCircle2 className="w-3 h-3" /> {new Date(n.naechsteSchulung).toLocaleDateString("de-DE")}
            </span>
          )}
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4 border-t border-border/30">
          <div className="grid grid-cols-2 gap-3 pt-4">
            {n.schulungsDatum && (
              <div className="bg-muted/30 rounded-xl px-3 py-2">
                <p className="text-xs text-muted-foreground">Schulungsdatum</p>
                <p className="text-sm font-bold">{new Date(n.schulungsDatum).toLocaleDateString("de-DE")}</p>
              </div>
            )}
            {n.naechsteSchulung && (
          ...
          {n.anbieter && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Schulungsanbieter / -leiter</p>
              <p className="text-sm text-foreground">{n.anbieter}</p>
            </div>
          )}

          {n.dokumentBase64 ? (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Dokument / Teilnahmebestätigung</p>
              {isPdf ? (
                <a href={n.dokumentBase64} target="_blank" rel="noopener noreferrer"
                ...
                <img src={n.dokumentBase64} alt="Dokument" className="w-full max-h-80 object-contain rounded-xl border border-border/40" />
              )}
            </div>
          ) : (
          ...
          {n.notizen && (
            <div>
              ...
              <p className="text-sm text-foreground">{n.notizen}</p>
            </div>
          )}

          <p className="text-xs text-muted-foreground">Hinzugefügt: {new Date(n.createdAt).toLocaleDateString("de-DE")}</p>
...
export default function Schulungsnachweise() {
  const { adminSession } = useAppStore();
  const isAdmin = !!adminSession;

  const [aktiveTab, setAktiveTab] = useState<Kategorie>("lebensmittelhygiene");
  const [daten, setDaten] = useState<Record<Kategorie, Schulungsnachweis[]>>({
    lebensmittelhygiene: [], arbeitssicherheit: [], brandschutz: [], haccp: [], sonstige: [],
  });
  ...
  const loadKategorie = useCallback(async (k: Kategorie) => {
    setLoading(true);
    try {
      const res = await fetch(`${BASE}/schulungsnachweise?tenantId=1&kategorie=${k}`);
  ...
  const handleSave = async (fields: Partial<Schulungsnachweis>) => {
    const res = await fetch(`${BASE}/schulungsnachweise`, {
      method: "POST",
  ...
  const handleDelete = async (id: number) => {
    await fetch(`${BASE}/schulungsnachweise/${id}`, { method: "DELETE" });
    setDaten((p) => ({ ...p, [aktiveTab]: p[aktiveTab].filter((n) => n.id !== id) }));
  };

  const tab = TABS.find((t) => t.key === aktiveTab)!;
  const liste = daten[aktiveTab];
  const abgelaufen = liste.filter((n) => getStatus(n.naechsteSchulung) === "abgelaufen").length;
  const bald = liste.filter((n) => getStatus(n.naechsteSchulung) === "bald").length;
  const gesamtAbgelaufen = Object.values(daten).flat().filter((n) => getStatus(n.naechsteSchulung) === "abgelaufen").length;
      ...
      <div className="max-w-3xl mx-auto space-y-4 pb-10">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">1.4 Schulungsnachweise</h1>
              <p className="text-xs text-muted-foreground">Nachweise aller Mitarbeiterschulungen und Unterweisungen</p>
            </div>
          </div>
          {!showForm && (
        ...
        {gesamtAbgelaufen > 0 && (
          <div className="flex items-center gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-2xl">
            ...
            <p className="text-sm font-bold text-red-700">
              {gesamtAbgelaufen} {gesamtAbgelaufen === 1 ? "Schulungsnachweis ist" : "Schulungsnachweise sind"} abgelaufen
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-5 gap-1 p-1 bg-muted/40 rounded-2xl border border-border/30">
          {TABS.map((t) => {
            ...
            const warn = tabDaten.filter((n) => getStatus(n.naechsteSchulung) === "abgelaufen").length;
            ...
            {abgelaufen > 0 && (
              <div className="flex items-center gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-500">Wiederholung erforderlich</p>
                </div>
              </div>
            )}
            {bald > 0 && (
        ...
        {tab.hinweis && (
          <div className={`flex items-start gap-3 px-4 py-3 ${tab.bgColor} border ${tab.borderColor} rounded-2xl`}>
            <BookOpen className={`w-4 h-4 mt-0.5 shrink-0 ${tab.color}`} />
            <p className={`text-xs leading-relaxed ${tab.color}`}>{tab.hinweis}</p>
          </div>
        )}

        {showForm && (
          <NachweisForm kategorie={aktiveTab} onSave={handleSave} onCancel={() => setShowForm(false)} />
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-xs text-muted-foreground mt-1">Klicken Sie auf "Hinzufügen" um den ersten Nachweis zu erfassen</p>
          </div>
        ) : (
          <div className="space-y-3">
            {liste.map((n) => (
              <NachweisKarte key={n.id} n={n} tab={tab} onDelete={() => handleDelete(n.id)} isAdmin={isAdmin} />
            ))}
          </div>
        )}
      </div>
