...
import { useWarenzustandOGStatus, useReinigungTaeglichStatus, useWareneingaengeStatus, useMetzgereiWareneingaengeStatus, useMetzgereiReinigungStatus, useKaesethekeStatus, type TrafficLight } from "@/hooks/useWarenzustandStatus";
  ...
  return (
    <div className="flex flex-col gap-0.5 py-1">
      {sections.filter((s) => !s.number.includes("_")).map((section) => {
        ...
        const trafficStatus: TrafficLight = section.number === "2.1" ? ogStatus : section.number === "2.2" ? reinigungStatus : section.number === "2.5" ? wareneingaengeStatus : section.number === "3.1" ? metzgereiStatus : section.number === "3.2" ? metzReinigungStatus : section.number === "3.4" ? kaesethekeStatus : "none";
        return (
          <Link
            key={section.id}
            href={href}
