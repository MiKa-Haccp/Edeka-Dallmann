...
import { MapPin, ShieldCheck, User as UserIcon, Bell } from "lucide-react";
import { useEffect } from "react";
import { Link } from "wouter";

export function Header() {
  const { data: markets, isLoading } = useListMarkets();
  const { selectedMarketId, setSelectedMarketId } = useAppStore();
    ...
    <header className="sticky top-0 z-40 w-full bg-white border-b border-border/60 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/90">
      <div className="flex h-16 items-center px-4 md:px-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 hover:bg-secondary p-1 pr-3 rounded-full transition-colors border border-transparent hover:border-border">
